 ...
 *
 */
public class BestPaintBoard extends SurfaceView {
...
//		invalidate();
	...
	*/
    ...
    // A. Thread 없이 단순히 View -> SurfaceView로 변경
    // 1. onDraw를 MyDraw로 수정하라
	// 2. 모든 invalidate를
	//	Canvas	canvas= holder.lockCanvas();
	//	if(canvas != null){
	//		myDraw(canvas);
	...
	//	}
	// 로 수정하라.
	// 3. onWindowFocusChanged 함수를 override하고, 이 함수에서 처음을 myDraw가 호출되도록 하라. 그이전에 만약 myDraw가 호출된다면 그것을 지워라
	//
	...
	// B. Thread 사용하기 SurfaceView -> SurfaceView implement SurfaceHolder.CallBack
	// 4. 앞에서 한 모든 myDraw호출을 없애라.
	// 5. Thread의 run 함수에서 주기적(초당 30번정도)으로 myDraw를 호출하라
